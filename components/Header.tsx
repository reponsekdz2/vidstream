  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-light-surface dark:bg-dark-bg z-50 h-16 flex items-center justify-between px-4 border-b border-light-element dark:border-dark-element">
      <div className="flex items-center gap-4">
        <button className="p-2 rounded-full hover:bg-light-element dark:hover:bg-dark-element">
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        {currentUser ? (
          <div ref={menuRef} className="relative">
            ...
            {isMenuOpen && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-light-surface dark:bg-dark-surface rounded-md shadow-lg py-1 z-50 border border-light-element dark:border-dark-element">
                <div className="px-4 py-3 border-b border-light-element dark:border-dark-element flex items-center gap-3">
                  <Avatar user={currentUser} size="sm"/>
                  <div>
                    <p className="font-semibold truncate">{currentUser.name}</p>
                    <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary truncate">{currentUser.email}</p>
                  </div>
                </div>
                <Link to="/my-channel" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm hover:bg-light-element dark:hover:bg-dark-element">
                  <UserCircleIcon className="w-5 h-5" />
                  My Channel
                </Link>
                <Link to="/creator-dashboard" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm hover:bg-light-element dark:hover:bg-dark-element">
                  <ChartBarIcon className="w-5 h-5" />
                  Creator Dashboard
                </Link>
                <Link to="/creator-dashboard/monetization" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm hover:bg-light-element dark:hover:bg-dark-element">
                  <CurrencyDollarIcon className="w-5 h-5" />
                  Monetization
                </Link>
                <Link to="/creator-dashboard/moderation" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm hover:bg-light-element dark:hover:bg-dark-element">
                ...
                <button onClick={handleLogout} className="flex items-center gap-3 w-full text-left px-4 py-2 text-sm hover:bg-light-element dark:hover:bg-dark-element">
